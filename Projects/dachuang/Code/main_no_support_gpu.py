...
parser.add_argument("-i",default=400,type=int,help="the num of in_features")
parser.add_argument("-e",type=int,default=200,help="epoch")
parser.add_argument("-g",type=int,default=4,help="g of g-gap encoding")
...
# g-gap特征表示
g_gap_pos = torch.Tensor(g_gap_dipeptide(pos,corpus_g_gap=corpus_g_gap,g_list=g))
g_gap_neg = torch.Tensor(g_gap_dipeptide(neg,corpus_g_gap=corpus_g_gap,g_list=g))
...
for epoch in range(Epoch):
    ...
    for i,data in enumerate(train_loader,0):
        x_1,x_2,labels = data
        x_1,x_2,labels = Variable(x_1).to(device),Variable(x_2).to(device),Variable(labels).to(device)
    ...
    if epoch % 1 == 0:
        ...
        precision,recall,f1,sn,sp,acc = Precision_test_gpu(g_gap_train,g_gap_test,model,device)
        if best_acc < acc:
